...
BEGIN
  -- 1) Plano com serviço contratado (Nutrição / Reabilitação) sem agendamento na vigência
  WITH alvo AS (
    SELECT pl.id AS plano_id, pl.aluno_id, pl.tipo, pl.data_inicio, pl.data_fim, s.servico, s.atividade
    ...
    SELECT 'agenda_servicos',
      ...
      'Plano ' || coalesce(f.tipo,'') || ' (início ' || to_char(f.data_inicio,'DD/MM/YYYY') || ') inclui ' ||
        ...
        ', mas não há nenhum agendamento desse serviço na vigência.',
      f.aluno_id,
  ...
  -- 2) Conflitos de profissional / sala (sobreposição real de horário, hoje até +30 dias)
  WITH ag AS (
...
REVOKE ALL ON FUNCTION public.fn_auditoria_fiscal_agenda_servicos() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_auditoria_fiscal_agenda_servicos() TO service_role;