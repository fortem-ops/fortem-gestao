  ...
  WITH calc AS (
    SELECT c.id AS credito_id, c.aluno_id, c.atividade, a.nome,
           ...
           coalesce(sum(CASE WHEN m.tipo = 'consumo' THEN m.quantidade
                             ...
                             -- Ajustes de USO (reconciliação "usado X -> Y"). Ajustes de
                             -- quantidade_inicial (correção de pacote) não entram aqui.
                             WHEN m.tipo = 'ajuste' AND m.observacao ILIKE '%usado % -> %' THEN m.quantidade
