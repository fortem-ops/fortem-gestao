-- 1. Novo status 'estornado' para cobrancas (aditivo, nenhuma linha alterada)
...
ALTER TABLE public.cobrancas ADD CONSTRAINT cobrancas_status_check
  CHECK (status = ANY (ARRAY['pendente'::text, 'pago'::text, 'atrasado'::text, 'cancelado'::text, 'isento'::text, 'estornado'::text]));

-- 2. Idempotencia de estorno em pagamentos_rede
ALTER TABLE public.pagamentos_rede ADD COLUMN IF NOT EXISTS idempotency_key text;
COMMENT ON COLUMN public.pagamentos_rede.idempotency_key IS 'Chave gerada pelo cliente no dialogo de estorno; impede que duplo clique gere dois refunds.';
CREATE UNIQUE INDEX IF NOT EXISTS uniq_pagamentos_rede_idempotency
  ON public.pagamentos_rede (idempotency_key)
  WHERE idempotency_key IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pagamentos_rede_tid_kind
  ON public.pagamentos_rede (tid, kind, status);

-- 3. Auditoria fiscal: cobranca estornada nao pode gerar achado falso
...
BEGIN
  WITH novos AS (
    ...
    WHERE c.data_vencimento >= current_date - interval '12 months'
      AND c.status NOT IN ('cancelado', 'estornado')
      AND (
...
-- 4. Saldo estornavel calculado no servidor (fonte da verdade)
CREATE OR REPLACE FUNCTION public.fn_cobranca_saldo_estornavel(_cobranca_id uuid)
RETURNS TABLE (
  cobranca_id uuid,
  tid text,
  valor_pago numeric,
  total_estornado numeric,
  saldo_estornavel numeric
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  SELECT
    ...
    c.tid,
    c.valor,
    coalesce(r.total, 0)::numeric / 100,
    greatest(c.valor - coalesce(r.total, 0)::numeric / 100, 0)
  FROM public.cobrancas c
  LEFT JOIN LATERAL (
...
GRANT EXECUTE ON FUNCTION public.fn_cobranca_saldo_estornavel(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_cobranca_saldo_estornavel(uuid) TO service_role;