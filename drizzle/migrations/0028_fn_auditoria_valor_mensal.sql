...
BEGIN
  ...
  -- 2) Valor da cobrança diferente do valor vigente (contrato e plano mensal)
  WITH novos AS (
    INSERT INTO public.auditoria_inconsistencias
      (categoria, subtipo, severidade, descricao, aluno_id, registros_afetados)
    SELECT
      ...
      'Cobrança de ' || to_char(c.valor, 'FM999999990.00') || ' (venc. ' ||
        ...
        to_char(ct.valor_cobrado, 'FM999999990.00') || ') e do valor mensal do plano ativo (' ||
    ...
    JOIN LATERAL (
      SELECT pl.id,
             CASE WHEN coalesce(pl.duracao_meses, 0) > 1 THEN pl.valor / pl.duracao_meses ELSE pl.valor END AS valor_mensal
