...
BEGIN
  ...
  IF NEW.tipo = 'plano' THEN
    ...
    IF COALESCE(_cat_plano.atividade, '') <> 'corrida' AND NEW.plano_id IS NOT NULL THEN
      _freq_sem := CASE
        WHEN COALESCE(_cat_plano.frequencia, '') = 'livre' THEN 5
        WHEN COALESCE(_cat_plano.frequencia, '') ~ '^[0-9]+x?$'
          THEN COALESCE(NULLIF(regexp_replace(_cat_plano.frequencia, '\D', '', 'g'), '')::smallint, 1)
        ELSE NULL
      END;
