...
RETURNS int LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT count(*)::int FROM generate_series(p_de + 1, p_ate, interval '1 day') d
  WHERE EXTRACT(isodow FROM d) < 6
    AND NOT EXISTS (SELECT 1 FROM public.ponto_feriados f WHERE f.data = d::date AND f.tipo::text <> 'facultativo');
$$;
REVOKE ALL ON FUNCTION public.fn_dias_uteis_entre(date, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fn_dias_uteis_entre(date, date) TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.fn_auditoria_fiscal_pipeline() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_auditoria_fiscal_pipeline() TO service_role;