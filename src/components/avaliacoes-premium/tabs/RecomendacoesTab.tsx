...
const PRIO_STYLE: Record<Recomendacao["prioridade"], { cls: string; label: string }> = {
  alta: { cls: "bg-rose-500/10 text-rose-600 border-rose-500/30", label: "ALTA" },
  media: { cls: "bg-amber-500/10 text-amber-600 border-amber-500/30", label: "MÉDIA" },
  baixa: { cls: "bg-emerald-500/10 text-emerald-600 border-emerald-500/30", label: "BAIXA" },
};
...
export function RecomendacoesTab({ recomendacoes }: Props) {
  if (recomendacoes.length === 0) {
    return (
      <div className="bio-card p-8 text-center text-[hsl(var(--bio-ink-muted))] text-sm flex flex-col items-center gap-2">
                ...
                return (
                  <div key={r.id} className={`bio-card p-4 border-l-2 ${prio === "alta" ? "border-l-rose-500/60" : prio === "media" ? "border-l-amber-500/60" : "border-l-emerald-500/60"}`}>
                    <div className="flex items-start gap-3">
                      <div className="p-2 rounded-md bg-[hsl(var(--bio-surface-2))]">
                        <Icon className="w-4 h-4 text-[hsl(var(--bio-ink-muted))]" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="text-sm font-semibold text-[hsl(var(--bio-ink))]">{r.titulo}</p>
