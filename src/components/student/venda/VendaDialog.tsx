...
export function VendaDialog({ alunoId, alunoNome, open, onOpenChange }: Props) {
  ...
  // Regra permanente: sempre amarrar ao `vendaId` via `origem_id`.
  // Usamos inserção condicional porque o índice único `idx_creditos_unicos_por_venda`
  // é parcial (WHERE ativo = true AND origem_id IS NOT NULL) e o Supabase JS client
  // não consegue usar ON CONFLICT em índices parciais.
  const criarCreditosServicos = async (svc: ServicosInclusos, vendaId: string) => {
    ...
    for (const linha of linhas) {
      // Verificar se já existe para evitar duplicata (índice parcial não suporta ON CONFLICT via JS client)
      const { data: existente } = await (supabase as any)
        .from("creditos_aluno")
        .select("id")
        .eq("aluno_id", alunoId)
        .eq("atividade", linha.atividade)
