...
export async function exportPTTP2PDF({ student, data, print }: ExportArgs): Promise<void> {
    ...
    const linhasHist: string[][] =
      lev.historico.length > 0
        ? lev.historico.map((h, i) => [
            ...
            h.data,
            `${h.peso} kg`,
            PTTP2_FASE_ESQUEMA[h.fase],
          ])
        : [["TREINO #1", "—", `${alvo.peso || "—"} kg`, PTTP2_FASE_ESQUEMA[lev.fase]]];
  ...
  ensurePage(6);
  doc.text(
    "Progressão automática: a cada sessão concluída o peso sobe 2,5% (mínimo 2,5 kg). A troca de fase (5-3-2 → 3-2 → 2 → teste de 1RM) é sempre manual.",
    mainX,
    y + 3,
  );

  const nome = `PTTP2-${student.nome.replace(/\s+/g, "-")}.pdf`;
  if (print) {
    doc.autoPrint();
