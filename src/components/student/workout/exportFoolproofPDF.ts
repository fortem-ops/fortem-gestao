...
export async function exportFoolproofPDF({ student, data, print }: ExportArgs): Promise<void> {
  ...
  data.levantamentos.forEach((lev) => {
    ...
    y = sectionBar(
      ...
      cicloConcluidoFP(lev)
        ...
        : `Próxima: ${alvo.esquema} @ ${alvo.peso || "—"} kg · +${lev.incremento} kg/semana`,
      mainX,
      y,
      mainW,
      6.0,
    ...
    const linhasHist: string[][] =
      lev.historico.length > 0
        ? lev.historico.map((h) => [
            ...
            h.data,
            `${h.peso} kg`,
            FP_FASE_ESQUEMA[h.fase],
          ])
        : [["SEMANA 1", "—", `${alvo.peso || "—"} kg`, FP_FASE_ESQUEMA[lev.fase]]];
    ...
    autoTable(doc, {
      ...
      head: [[
        ...
        { content: "FASE", styles: { halign: "center" as const } },
      ]],
      body: linhasHist,
      styles: commonStyles,
      headStyles: commonHeadStyles,
  ...
  ensurePage(6);
  doc.text(
    "Peso inicial: 70% do 1RM. A cada sessão principal concluída o peso sobe um valor fixo (2,5% do 1RM de referência, mínimo 2,5 kg). A sessão de hipertrofia não progride. A troca de fase (5x5 → 3x3 → 2x2) e o encerramento do ciclo são sempre manuais.",
    mainX,
    y + 3,
    { maxWidth: mainW },
  );

  const nome = `Foolproof-${student.nome.replace(/\s+/g, "-")}.pdf`;
  if (print) {
    doc.autoPrint();
