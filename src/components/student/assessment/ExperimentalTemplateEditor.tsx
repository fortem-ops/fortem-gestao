...
export function ExperimentalTemplateEditor({ open, onOpenChange }: Props) {
  const { user } = useAuth();
  ...
  const [schema, setSchema] = useState<ExperimentalSchema>({ sections: [] });
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (open && initial) setSchema(JSON.parse(JSON.stringify(initial)));
  }, [open, initial]);
  ...
  const addSection = () =>
    setSchema((s) => ({
      ...s,
      sections: [...s.sections, { id: crypto.randomUUID(), title: "Nova seção", questions: [] }],
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await saveExperimentalSchema(schema, user.id);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar formulário Experimental</DialogTitle>
        </DialogHeader>
...
function Textarea2({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <textarea
      value={value}
