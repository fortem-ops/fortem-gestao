...
export default function PortalWorkouts() {
  ...
  // ── Easy Strength: renderização dedicada ────────────────────
  if (
    treino &&
    ((treino as any).template_fase === EASY_STRENGTH_LABEL ||
      isEasyStrengthContent(treino.conteudo))
  ) {
    return (
  ...
  if (
    treino &&
    ((treino as any).template_fase === FOOLPROOF_LABEL || isFoolproofContent(treino.conteudo))
  ) {
    return (
      <PortalFoolproofView
        treino={treino}
  ...
  if (
    treino &&
    ((treino as any).template_fase === PTTP_LABEL || isPTTPContent(treino.conteudo))
  ) {
    return (
      <PortalPTTPView
        treino={treino}
...
// ─────────────────────────────────────────────────────────────
// Portal Power to the People: progressão viva por resultado real
// ─────────────────────────────────────────────────────────────

function PortalPTTPView({
  treino,
  sessoes,
  student,
  agendamentoHoje,
  ...
  const data = (treino?.conteudo ?? null) as PTTPConteudo | null;
  ...
  const levsRampa = data.levantamentos.filter((l) => l.modo === "rampa");
  ...
  async function handleConcluir(slot: string) {
    ...
    const faltando = levsRampa.some((l) => resultados[l.levantamento] === undefined);
    if (faltando) {
      toast.error("Informe o resultado das 2 séries de cada levantamento central.");
      return;
    }
    setConcluindoSlot(slot);
    try {
      const hoje = format(new Date(), "yyyy-MM-dd");
      const novoConteudo: PTTPConteudo = {
        ...data,
        levantamentos: data.levantamentos.map((l) =>
          l.modo === "rampa"
            ? registrarResultadoPTTP(l, resultados[l.levantamento] === true, hoje)
            : l,
  ...
  const setResultado = (lev: PTTPLevantamento, ok: boolean) =>
    setResultados((p) => ({ ...p, [lev]: ok }));
  ...
  return (
    <div className="p-4 space-y-5 max-w-3xl mx-auto pb-16">
      <header className="space-y-1">
        <h1 className="text-lg font-bold">{PTTP_LABEL}</h1>
        <p className="text-xs text-muted-foreground">
          {data.frequencia} treinos por semana · 2 levantamentos centrais em todos os treinos
        </p>
      </header>
      ...
      {/* Levantamentos centrais — resultado da sessão */}
      <section className="rounded-xl border border-border overflow-hidden">
        <div className="px-3 py-2 bg-muted/60">
          <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
        ...
        <div className="p-3 space-y-3">
          {data.levantamentos.map((lev) => {
            ...
            const escolha = resultados[lev.levantamento];
            return (
              <div key={lev.levantamento} className="border rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <span className="tabular-nums shrink-0">
                    {alvo.esquema} ·{" "}
                    <span className="font-semibold">{alvo.peso || "—"} kg</span>
                  </span>
                </div>
                {lev.modo === "manutencao" ? (
                  <p className="text-[11px] text-muted-foreground italic">
                    Em manutenção — sem registro de resultado.
                  </p>
                ) : (
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      onClick={() => setResultado(lev.levantamento, true)}
                      className={
                    ...
                    >
                      Completei as 2 séries
                    </button>
                    <button
                      onClick={() => setResultado(lev.levantamento, false)}
                      className={
                        "py-2 rounded-lg text-xs font-semibold border transition-colors " +
                        (escolha === false
...
function PortalFoolproofView({
  ...
  const data = (treino?.conteudo ?? null) as FoolproofConteudo | null;
  ...
  const slots = fpSlots(data.diasTreinoSemana);
  const concluidasPorSlot = (slot: string) =>
    sessoes.filter((s) => s.concluido_em && s.variacao === slot).length;
  ...
  async function handleConcluir(slot: string) {
    ...
    try {
      const hoje = format(new Date(), "yyyy-MM-dd");
      // Só a sessão principal daquele dia faz o peso subir.
      const novoConteudo: FoolproofConteudo = {
        ...data,
        levantamentos: data.levantamentos.map((l) =>
          l.slotPrincipal === slot ? registrarSessaoPrincipalFP(l, hoje) : l,
        ),
      };
      const { error } = await supabase
        .from("treinos")
  ...
  return (
    <div className="p-4 space-y-5 max-w-3xl mx-auto pb-16">
      <header className="space-y-1">
        <h1 className="text-lg font-bold">{FOOLPROOF_LABEL}</h1>
        <p className="text-xs text-muted-foreground">
          {slots.length} treinos por semana · {data.levantamentos.length} levantamentos
        </p>
      </header>
            ...
            return (
              <div key={lev.levantamento} className="border rounded-lg p-3 space-y-1">
                <div className="flex items-center justify-between gap-2 text-xs">
                  ...
                  <span className="tabular-nums shrink-0">
                    {alvo.concluido ? (
                      <span className="font-semibold">Ciclo concluído</span>
                    ) : (
                      <>
                        {lev.slotPrincipal} · {alvo.esquema} ·{" "}
                        <span className="font-semibold">{alvo.peso || "—"} kg</span>
                      </>
                    )}
                  </span>
                ...
                <p className="text-[11px] text-muted-foreground">
                  {cicloConcluidoFP(lev)
                    ? `Ciclo concluído${lev.rm1Final ? ` — 1RM final: ${lev.rm1Final} kg` : ""}`
                    : FP_FASE_LABEL[lev.fase]}
                </p>
                {hiper && (
                  <p className="text-[11px] text-muted-foreground">
                    Hipertrofia ({lev.hipertrofia.slot}): {hiper.esquema} · {hiper.peso || "—"} kg
                  </p>
                )}
              </div>
            );
      ...
      {slots.map((slot, i) => {
        const sessoesDoSlot = fpLevantamentosDoSlot(data, slot);
        return (
          <section key={slot} className="rounded-xl border border-border overflow-hidden">
            <div className="px-3 py-2 bg-muted/60 flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            ...
            <div className="p-3 space-y-3">
              <div className="space-y-1">
                {sessoesDoSlot.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Sem levantamentos principais neste dia.
                  </p>
                )}
                {sessoesDoSlot.map((s, k) => {
...
function PortalEasyStrengthView({
  treino,
  sessoes,
  student,
  agendamentoHoje,
  ...
  const data = (treino?.conteudo ?? null) as EasyStrengthConteudo | null;
  ...
  return (
    <div className="p-4 space-y-5 max-w-3xl mx-auto pb-16">
      <header className="space-y-1">
        <h1 className="text-lg font-bold">{EASY_STRENGTH_LABEL}</h1>
        <p className="text-xs text-muted-foreground">
          {slots.length} sessões por semana · {data.levantamentos.length} levantamentos em todas as
          sessões
        </p>
      </header>
      ...
      {slots.map((slot, i) => {
        ...
        const semana = semanaAtualES(concluidasPorSlot(slot));
        ...
        return (
          <section key={slot} className="rounded-xl border border-border overflow-hidden">
            <div className="px-3 py-2 bg-muted/60 flex items-center justify-between">
              ...
              <span className="text-[11px] text-muted-foreground tabular-nums">
                Semana {semana}/{ES_TOTAL_SEMANAS}
              </span>
            </div>
            <div className="p-3 space-y-3">
              <div className="space-y-1">
                {data.levantamentos.map((l, k) => {
                  ...
                  return (
                    <div key={k} className="flex items-center justify-between text-xs gap-2">
                      <span className="truncate flex items-center gap-1 min-w-0">
                        <span className="font-semibold shrink-0">{l.levantamento}</span>
                        <span className="text-muted-foreground truncate">
                          {cleanName(base.nome)}
                        </span>
                        {base.video_url && (
                      ...
                      <span className="tabular-nums shrink-0">
                        {plano.esquema} ·{" "}
                        <span className="font-semibold">{kgES(l.rm1, plano.pct) || "—"} kg</span>
                      </span>
                    </div>
                  );
                })}
