...
BEGIN
  ...
  v_qtd_aval := COALESCE(NULLIF(p_servicos_inclusos->>'avaliacao_funcional', '')::int, 0);
  v_qtd_nutri := COALESCE(
    NULLIF(p_servicos_inclusos->>'nutricao', '')::int,
    NULLIF(p_servicos_inclusos->>'consultas_nutricao', '')::int,
    0
  );
  v_qtd_reab := COALESCE(
    NULLIF(p_servicos_inclusos->>'reabilitacao', '')::int,
    NULLIF(p_servicos_inclusos->>'consultas_reabilitacao', '')::int,
    0
  );
  ...
  IF v_qtd_aval > 0 THEN
    ...
    VALUES (v_credito_id, 'compra', v_qtd_aval, auth.uid(), 'Serviço incluso no contrato recorrente: Avaliação Funcional');
  END IF;
  ...
  IF v_qtd_nutri > 0 THEN
    ...
    INSERT INTO public.creditos_movimentos (credito_id, tipo, quantidade, registrado_por, observacao)
    VALUES (v_credito_id, 'compra', v_qtd_nutri, auth.uid(), 'Serviço incluso no contrato recorrente: Nutrição');
  END IF;
  ...
  IF v_qtd_reab > 0 THEN
    ...
    INSERT INTO public.creditos_movimentos (credito_id, tipo, quantidade, registrado_por, observacao)
    VALUES (v_credito_id, 'compra', v_qtd_reab, auth.uid(), 'Serviço incluso no contrato recorrente: Reabilitação');
  END IF;
...
REVOKE ALL ON FUNCTION public.fn_criar_contrato_recorrencia(uuid, uuid, uuid, numeric, numeric, date, text, uuid, boolean, jsonb) FROM PUBLIC;
