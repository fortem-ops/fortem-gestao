...
BEGIN
  ...
  IF v_current_stage = v_experimental_stage AND v_prospect_stage IS NOT NULL THEN
    ...
    VALUES (v_aluno_id, v_experimental_stage, v_prospect_stage, 'system', 'Presença confirmada no Treino Experimental');
  END IF;
...
DROP TRIGGER IF EXISTS trg_presenca_experimental_to_prospect ON public.agenda_presencas;
CREATE TRIGGER trg_presenca_experimental_to_prospect
AFTER INSERT OR UPDATE ON public.agenda_presencas
FOR EACH ROW EXECUTE FUNCTION public.trigger_presenca_experimental_to_prospect();
