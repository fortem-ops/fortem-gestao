...
BEGIN
  ...
  IF _max_abs > _tol_marc OR _soma_abs > _tol_dia THEN
    ...
    -- Intervalo: apenas excesso gera desconto. Intervalo nao realizado NAO gera hora extra.
    IF _div_i > 0 THEN
...
BEGIN
  ...
  END LOOP;
  FOR _id IN SELECT id FROM public.ponto_jornadas WHERE saida IS NULL ORDER BY data
  LOOP
    PERFORM public.fn_ponto_calcular_divergencias(_id);
  END LOOP;
END $$;