...
BEGIN
  -- Bypass para reprocessamento administrativo
  IF current_setting('app.bypass_fechamento_lock', true) = 'true' THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  -- Construir SQL da função corrigida com timezone
  ...
  -- Ativar bypass do trigger de bloqueio de fechamento aprovado
  PERFORM set_config('app.bypass_fechamento_lock', 'true', true); -- true = LOCAL (só esta transação)
  ...
  -- Reconsolidar banco de horas
  PERFORM public.fn_ponto_consolidar_banco(id)
  FROM public.ponto_jornadas WHERE saida IS NOT NULL;

  -- Desativar bypass (redundante pois é LOCAL, mas explícito)
