...
BEGIN
  ...
  SELECT horario_inicio, horario_fim, intervalo_min
  INTO _h
  FROM public.ponto_horarios_professor
  WHERE usuario_id = _j.usuario_id
    AND dia_semana = _dow
    AND ativo = true
  LIMIT 1;

  IF _h.horario_inicio IS NOT NULL THEN
    -- Os horários da escala são horários civis locais de Porto Alegre.
    -- A conversão explícita evita que a sessão UTC acrescente três horas à divergência.
    _prev_entrada := (_j.data + _h.horario_inicio) AT TIME ZONE 'America/Sao_Paulo';
    _prev_saida := (_j.data + _h.horario_fim) AT TIME ZONE 'America/Sao_Paulo';
  ...
  IF _prev_int_min IS NOT NULL
    AND _j.intervalo_inicio IS NOT NULL
    AND _j.intervalo_fim IS NOT NULL THEN
  ...
  ELSIF _excedida THEN
    IF _descont > 0 AND _extras = 0 THEN
      _status := 'banco_negativo';
    ELSIF _extras > 0 AND _descont = 0 THEN
      _status := 'hora_extra';
    ELSE
      _status := 'divergencia_considerada';
    END IF;
  ...
  SET prev_entrada = _prev_entrada,
