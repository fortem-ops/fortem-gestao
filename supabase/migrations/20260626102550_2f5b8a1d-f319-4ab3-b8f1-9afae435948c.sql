CREATE OR REPLACE FUNCTION public.fn_criar_contrato_recorrencia(
  p_venda_id uuid, p_aluno_id uuid, p_plano_id uuid,
  p_valor_mensal numeric, p_taxa_mensal numeric,
  p_data_inicio date, p_forma_pagamento text,
...
BEGIN
  ...
  -- Mapear plano_tipo a partir do nome do plano (compatível com CHECK constraint)
  v_plano_tipo := CASE
  ...
  -- Criar créditos de serviços inclusos
  ...
  IF v_qtd_aval > 0 THEN
    INSERT INTO public.creditos_aluno (aluno_id, atividade, quantidade_inicial, quantidade_restante, origem, contrato_id)
    VALUES (p_aluno_id, 'avaliacao_funcional', v_qtd_aval, v_qtd_aval, 'contrato', v_contrato_id);
  END IF;
  IF v_qtd_nutri > 0 THEN
    INSERT INTO public.creditos_aluno (aluno_id, atividade, quantidade_inicial, quantidade_restante, origem, contrato_id)
    VALUES (p_aluno_id, 'nutricao', v_qtd_nutri, v_qtd_nutri, 'contrato', v_contrato_id);
  END IF;
  IF v_qtd_reab > 0 THEN
    INSERT INTO public.creditos_aluno (aluno_id, atividade, quantidade_inicial, quantidade_restante, origem, contrato_id)
    VALUES (p_aluno_id, 'reabilitacao', v_qtd_reab, v_qtd_reab, 'contrato', v_contrato_id);
  END IF;
