-- 1. Adicionar nova coluna multi-nível
ALTER TABLE public.beneficios
  ADD COLUMN niveis_permitidos public.clube_nivel_membro[] NOT NULL DEFAULT '{}';

-- 2. Backfill: para cada benefício existente, preencher com todos os níveis >= nivel_minimo
--    Hierarquia: start < start_plus < power < pro < max  (agregador é fora da hierarquia)
UPDATE public.beneficios SET niveis_permitidos = (
  SELECT array_agg(n::public.clube_nivel_membro)
  FROM unnest(ARRAY['start','start_plus','power','pro','max']) AS n
  WHERE array_position(ARRAY['start','start_plus','power','pro','max'], n)
        >= array_position(ARRAY['start','start_plus','power','pro','max'], nivel_minimo::text)
);

-- Garantir pelo menos um nível em todos
UPDATE public.beneficios SET niveis_permitidos = ARRAY['start','start_plus','power','pro','max']::public.clube_nivel_membro[]
  WHERE niveis_permitidos IS NULL OR array_length(niveis_permitidos,1) IS NULL;

-- 3. Remover coluna antiga
ALTER TABLE public.beneficios DROP COLUMN nivel_minimo;

-- 4. Atualizar função de validação para usar niveis_permitidos
