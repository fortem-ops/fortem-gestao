...
BEGIN
  ...
  -- Construir o SQL da função corrigida como texto simples (sem dollar-quoting aninhado)
  _sql := 'CREATE OR REPLACE FUNCTION public.fn_ponto_calcular_divergencias(_jornada_id uuid) '
    || 'RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO ' || quote_literal('public') || ' AS '
    || quote_literal(
      'DECLARE '
  ...
  -- Limpar lançamentos automáticos incorretos
  DELETE FROM public.ponto_banco_horas
  WHERE tipo IN ('tolerancia_excedida', 'hora_extra')
    AND referencia_jornada_id IS NOT NULL;

  -- Desativar trigger de proteção de fechamento aprovado temporariamente e bypass de RLS
  SET LOCAL row_security = off;
  ALTER TABLE public.ponto_jornadas DISABLE TRIGGER ALL;

  -- Reprocessar todas as jornadas com saída
  ...
  -- Reativar trigger
  ALTER TABLE public.ponto_jornadas ENABLE TRIGGER ALL;

  RETURN 'ok:' || _count::text || ' jornadas reprocessadas';
END;
$wrapper$;
