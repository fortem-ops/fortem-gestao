ALTER TABLE public.produtos_catalogo ADD COLUMN IF NOT EXISTS permite_encomenda boolean NOT NULL DEFAULT false;
ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS eh_encomenda boolean NOT NULL DEFAULT false;

ALTER TABLE public.estoque_movimentos DROP CONSTRAINT IF EXISTS estoque_movimentos_tipo_check;
ALTER TABLE public.estoque_movimentos ADD CONSTRAINT estoque_movimentos_tipo_check
  CHECK (tipo = ANY (ARRAY['entrada'::text,'saida'::text,'ajuste'::text,'reserva'::text,'cancelamento_reserva'::text,'encomenda'::text]));
...
BEGIN
  IF _idem IS NOT NULL THEN
    ...
    IF FOUND THEN
      RETURN jsonb_build_object('ok', true, 'reused', true,
        ...
        'status', _existente.status, 'eh_encomenda', _existente.eh_encomenda);
    END IF;
  END IF;
  ...
  FOR _item IN SELECT * FROM jsonb_array_elements(_itens) LOOP
    ...
    _preco := NULL; _estoque := NULL; _permite := NULL;
    ...
    IF _estoque < _qtd AND coalesce(_permite, false) = false THEN
      RETURN jsonb_build_object('ok', false, 'error', 'estoque_insuficiente',
        'variante_id', _variante_id);
    END IF;
  ...
  IF _eh_encomenda THEN
    UPDATE public.pedidos SET eh_encomenda = true WHERE id = _pedido_id;
  END IF;

  RETURN jsonb_build_object('ok', true, 'reused', false,
    'pedido_id', _pedido_id, 'valor_final', _total,
    'status', 'aguardando_pagamento', 'eh_encomenda', _eh_encomenda);
END;
$function$;
...
BEGIN
  ...
  PERFORM set_config('app.estoque_mov', 'on', true);
  -- devolve somente o que foi efetivamente reservado (ignora itens de encomenda)
  FOR _r IN
    SELECT variante_id, sum(quantidade)::int AS quantidade
    FROM public.estoque_movimentos
    WHERE pedido_id = p_pedido_id AND tipo = 'reserva'
...
REVOKE EXECUTE ON FUNCTION public.fn_loja_reverter_reserva(uuid) FROM PUBLIC, anon, authenticated;
